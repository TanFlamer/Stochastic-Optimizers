...
for dim = [2,3,5,10,20]
  ...
  while toc < 30  % at least 30 seconds
    % PSO(@fgeneric, dim, ftarget, 1e5);  % adjust maxfunevals
    % DE(@fgeneric, dim, ftarget, 1e5);  % adjust maxfunevals
    % JADE(@fgeneric, dim, ftarget, 1e5);  % adjust maxfunevals
    DEAE(@fgeneric, dim, ftarget, 1e5);  % adjust maxfunevals
    nbrun = nbrun + 1;
  end  % while
