% The following source code was extracted from
% https://numbbo.github.io/data-archive/bbob/ 
% Number: 067 
% Algorithm Name: DEAE
% Author: Pošík and Klemš
% [Accessed 6 April 2022] and modified

function [exitcode] = DEAE(FUN, DIM, ftarget, maxfunevals)
    
    %-------- setDE --------------------------
    setDE.popsize = '20';  % population size 
    setDE.maxpopsize = inf; % max population size
    ...
    setDE.mutation_op = 'best';   % mutation operator
    setDE.crossover_op = 'bin';   % crossover operator
    setDE.CR = 0.7; % crossover probability
    ...
    %-------- setJA ---------------------------
    setJA.adaptJA_muF = false;  % use adaptation of mu_F (mutation parameter)
    setJA.adaptJA_muCR = false; % use adaptation of mu_CR (crossover parameter)
    ...
    setJA.JA_pArch = 0.1;   % how much best individuals to store in archive (percentage, 0 = switch off)
    setJA.JA_pMut = 0.1;    % from 100*JA_pMut% of best individual are randomly picked individual in JA-mutation 
    setJA.JA_c_CR = 0.1; % muCR learning rate
    setJA.JA_c_F = 0.1;  % muF learning rate
    ...
    setAE.mu = 'ceil(popsize/2)'; 
    setAE.CCOVFAC = 8;
    setAE.c_p = '1/sqrt(AE.DIM)';    % learning rate for the evolution path 
    setAE.c_1 = '0.1';   % learning rate for rank-ONE-update (evolution path) 
    setAE.c_mu = '0.1';   % learning rate for rank-MU-update (covariance matrix estimation)
    %------------------------------------------
    ...
    popsize = min(eval(setDE.popsize), setDE.maxpopsize); % set parental population size 
    ...
    crossover_op = setDE.crossover_op;   % type of crossover operator
    mutation_op = setDE.mutation_op;   % type of mutation operator
    ...
    JA_pMut = setJA.JA_pMut;% from 100*JA_pMut% of best individuals are randomly picked individuals in JA-mutation
